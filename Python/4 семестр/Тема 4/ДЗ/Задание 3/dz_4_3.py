...
def read_labyrinth(filename):
    ...
    for i, line in enumerate(lines):
        if len(line) != M:
            print(
                f"Ошибка: Строка {i+1} имеет длину {len(line)}, ожидается {M}")
            sys.exit(1)
    ...
    for i, line in enumerate(lines):
        ...
        for j, ch in enumerate(line.upper()):
            if ch not in "SFWO":
                print(f"В строке {i+1}, столбце {j+1} найден символ {ch}")
                sys.exit(1)

            if ch == "S":
                if start is not None:
                    print("Ошибка: Найдено несколько входов!")
                    sys.exit(1)
                start = (i, j)
            elif ch == "F":
                if finish is not None:
                    print("Ошибка: Найдено несколько выходов!")
                    sys.exit(1)
                finish = (i, j)
